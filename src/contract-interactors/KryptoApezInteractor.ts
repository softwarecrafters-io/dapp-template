...
export class KryptoApezInteractor {
	...
	getAllNfts() {
		console.log('getallnftsl');
		return from(this.walletService.connectToMetamask()).pipe(mergeMap(_ => from(this.contract.getKryptoApez())));
	}
	...
	totalSupply() {
		return from(this.walletService.connectToMetamask()).pipe(mergeMap(_ => from(this.contract.totalSupply())));
	}
